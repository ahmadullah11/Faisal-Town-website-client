      ...
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-sm text-gray-600 leading-relaxed max-w-2xl mx-auto">
            ...
            modern amenities, and exceptional customer service.
          </p>
        </div>
        ...
        <div className="relative">
                ...
                <div key={review.id} className="w-full flex-shrink-0 px-2">
                  <div className="bg-white rounded-xl p-6 shadow-md mx-auto max-w-xl text-center">
                    ...
                    <div className="flex flex-col items-center space-y-2">
                      ...
                      />
                      <h4 className="text-base font-semibold text-gray-900">{review.name}</h4>
                      <p className="text-xs text-gray-600">
                        {review.designation} • Since {review.joinedYear}
                      </p>
          ...
          {/* Navigation */}
          <button onClick={prevSlide} className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-3 w-8 h-8 bg-white rounded-full shadow flex items-center justify-center text-gray-600 hover:text-amber-600">
            <ChevronLeft className="w-4 h-4" />
          </button>
          <button onClick={nextSlide} className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-3 w-8 h-8 bg-white rounded-full shadow flex items-center justify-center text-gray-600 hover:text-amber-600">
        ...
        <div className="flex justify-center space-x-1 mt-4">
          {reviews.map((_, i) => (
            <button
              key={i}
              onClick={() => goToSlide(i)}
              className={`h-2 rounded-full transition-all ${i === currentSlide ? 'bg-amber-500 w-6' : 'bg-gray-300 w-2'}`}
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-10">
          {[
            ...
            <div key={i} className="text-center p-4 bg-white rounded-xl shadow">
